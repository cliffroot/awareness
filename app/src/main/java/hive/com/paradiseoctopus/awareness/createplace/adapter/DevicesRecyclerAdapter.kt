...
class DevicesRecyclerAdapter(val context: Context, val scans: List<ScanResult>, val resultObservable : PublishSubject<Int>,
    ...
    override fun onBindViewHolder(holder: ViewHolder?, position: Int) {
        ...
        holder?.icon?.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.wifi_black))
        ...
        (holder?.itemView)?.setOnClickListener { holder?.radioButton?.isChecked = true }
    }

    override fun getItemCount(): Int = scans.count()

    class ViewHolder(v : View) : RecyclerView.ViewHolder(v) {
        val icon = v.findViewById(R.id.device_icon) as ImageView
        val ssid = v.findViewById(R.id.device_ssid)  as TextView
        val radioButton = v.findViewById(R.id.selected_device) as RadioButton
